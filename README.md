...
- 短按角色：内置 Live2D 模型会触发现有骨骼动作与表情；静态模型或动作忙碌时使用跳跃、压扁回弹、左右抖动作为回退反馈，并避免连续重复同一种互动。
- 互动时有概率显示一句符合角色气质的简短中文气泡；气泡带有冷却时间，并使用模型清单声明的空白安全区，不遮挡角色。
...
- **成长与收藏**：有效互动会累计终身次数并显示桌宠旁 HUD；20 / 100 / 500 / 1000 次设有固定模型奖励，每 50 次还有 35% 的本地随机惊喜，连续两次未获得奖励后第三次保底。奖励不重复、不可交易，普通统计清零不会影响成长；设置页可单独重置成长与收藏。
...
从旧版升级的本地档案会自动保留此前可用的全部内置模型；全新档案从默认小八开始成长。用户自行导入的模型不参与奖励锁定，始终可以直接使用。
...
```text
target/x86_64-pc-windows-msvc/release/bundle/nsis/BlueWhitePet_{version}_x64-setup.exe
```
